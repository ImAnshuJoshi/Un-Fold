...
const categoryz = (c) => `<a href='../Category/?id=${c.id}'><div class="cat first">
<div class="cat-img">
...
const categoriesfunc = async () => {
  ...
  categoryg.map(async (c) => {
    document.getElementById('category-id').insertAdjacentHTML('afterbegin', categoryz(c))
  })
}
window.onload = async () => {
  ...
  if (!followingblogsj.length) {
    document.getElementsByClassName('latest-cards row')[0].insertAdjacentHTML('afterbegin', `<h2 style="color:black;margin:5%;" >You don't follow any user yet... Follow users to see their latest blogs</h2>`)
  } else {
    ...
    Promise.all(fblogs).then(function (res) {
      ...
      res.forEach((b) => {
        document.getElementsByClassName('latest-cards row')[0].insertAdjacentHTML('afterbegin', blogCard(b.imageurl, b.title, b.content, b.user, b.id, b.tags, b.K))
  ...
  Promise.all(allblogs).then(function (res) {
    res.sort((a, b) => a.createdAt - b.createdAt)
    res.forEach((b) => {
      document.getElementById('i1').insertAdjacentHTML('afterbegin', blogCard(b.imageurl, b.title, b.content, b.user, b.id, b.tags, b.K))
      const text = document.getElementsByClassName(`desc2 ${i - 1}`)[0].innerText
      document.getElementsByClassName(`desc2 ${i - 1}`)[0].innerHTML = text.substring(0, 50) + '.....'
    })
  })
