...
const blogCard = (blog, user, tags, K) => `<a href='../Blog/?id=${blog.id}' style="text-decoration:none;">
...
const searchBlogs = async () => {
  ...
  blogs.forEach(async (blog) => {
    if (blog.title.toUpperCase().includes(word.toUpperCase())) {
      ...
      console.log(blog.imageurl, blog.title, blog.content, user, tags)
      let K = false
      if (bmarkedblogsk.includes(blog.id)) K = true
      document.getElementById('search').insertAdjacentHTML('afterbegin', blogCard(blog, user, tags, K))
    }
  })
}
...
const searchUsers = async () => {
  ...
  allusers.forEach((user) => {
    if (user.firstName.toUpperCase().includes(word.toUpperCase()) || user.lastName.toUpperCase().includes(word.toUpperCase())) {
      ...
      document.getElementById('search-user-id').insertAdjacentHTML('afterbegin', userz(user.imageurl, user.firstName, user.lastName, user.id))
    }
  })
}
