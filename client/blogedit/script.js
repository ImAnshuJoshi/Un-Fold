...
window.onload = async() => {
  ...
  const currentblog= await fetch(
    "http://65.0.100.50/api/blog/getblogbyid?"+new URLSearchParams({ id: id }),
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
button.addEventListener("click", async () => {
  ...
  const editedblog = await fetch(
    "http://65.0.100.50/api/blog/editBlog?" + new URLSearchParams({ id: id }),
    {
      method: "PUT",
      body: formdata,
      headers: {},
  ...
  if (editedblog.status !== 200) console.log(editedblog.message);
    ...
    location.href = `client/Blog-opening/blog-opening.html?id=${id}`;
  }
